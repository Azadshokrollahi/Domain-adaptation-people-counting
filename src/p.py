...
def _create_single_method_efficiency_plot(sizes, values, method, save_path,
                                          scenario_name, model_type, ws):
    """
    Efficiency plot for a single method with peak annotation arrow.
    Carried over from progressive_feature_extraction.py.
    """
    if len(sizes) == 0:
        return
    style = METHOD_STYLES.get(method, {})
    ...
    fig, ax = plt.subplots(figsize=(_fig_width(len(sizes)), 7))
    ax.plot(sizes, values, 'o-', lw=2, ms=5,
            color=style.get('color', 'gray'), zorder=5)
    ax.axvline(x=peak_size, color='red', linestyle='--', alpha=0.7, lw=2,
               label=f'Peak efficiency: size={peak_size} (eff={peak_val:.2f})')
    _setup_linear_xaxis(ax, sizes)
    ax.set_xlabel('Training Data Size', fontsize=13, fontweight='bold')
    ax.set_ylabel('Efficiency (Improvement% / Data%)', fontsize=13, fontweight='bold')
    ax.set_title(f'Training Efficiency — {method}\n{scenario_name} | {model_type} | ws={ws}',
                 fontsize=14, fontweight='bold')
...
def create_learning_curves(df, out_dir, model_type, ws, scenario_name, zs_mae):
    """
    Create wide learning-curve images per metric.

    Produces three categories of outputs:
      A. Combined plots (both methods overlaid) — 6 metrics each
      B. Per-method individual metric plots — 6 metrics × 2 methods
      C. Per-method efficiency plots with peak-annotation arrow
    """
    ...
    # ── A. COMBINED PLOTS (both methods overlaid) ──────────────────────────

    # A1. MAE — with zero-shot reference line
    ...
    if improv_series:
        ...
        if all_finals:
            ...
            if best_final > 0:
                y_hlines.append((0.9 * best_final,
                                 f'90% of max ({0.9 * best_final:.1f}%)',
                                 'orange', '-.'))
                y_hlines.append((0.8 * best_final,
                                 f'80% of max ({0.8 * best_final:.1f}%)',
                                 'cyan', ':'))
    create_two_method_metric_plot(
    ...
    # A6. Efficiency (combined — both methods on one plot)
    eff_series = {}
    for method in METHODS:
        sub = df_trained[df_trained['method'] == method].sort_values('train_size')
        if len(sub) == 0:
    ...
    # ── B. PER-METHOD INDIVIDUAL PLOTS ─────────────────────────────────────
    # One image per method × metric for detailed per-method inspection.
    ...
    for method in METHODS:
        ...
        m_slug = method.lower().replace(' ', '_').replace('-', '_')

        # B1. MAE
        series_single = {method: (sizes, sub['test_mae'].values)}
        create_two_method_metric_plot(
            series_single, 'MAE', f'MAE — {method}',
            os.path.join(out_dir, f'indiv_MAE_{m_slug}_{model_type}_ws{ws}.png'),
            scenario_name, model_type, ws,
            y_hlines=[(zs_mae, f'Zero-Shot: {zs_mae:.4f}', 'gray', '--')]
                      if not np.isnan(zs_mae) else None)

        # B2. RMSE
        create_two_method_metric_plot(
            {method: (sizes, sub['test_rmse'].values)},
            'RMSE', f'RMSE — {method}',
            os.path.join(out_dir, f'indiv_RMSE_{m_slug}_{model_type}_ws{ws}.png'),
            scenario_name, model_type, ws)

        # B3. R²
        r2_vals = sub['test_r2'].values
        create_two_method_metric_plot(
            {method: (sizes, r2_vals)},
            'R²', f'R-Squared — {method}',
            os.path.join(out_dir, f'indiv_R2_{m_slug}_{model_type}_ws{ws}.png'),
            scenario_name, model_type, ws,
            y_lim=[max(-0.1, r2_vals.min() - 0.05), 1.0])

        # B4. Accuracy ±1
        create_two_method_metric_plot(
            {method: (sizes, sub['test_acc_pm1'].values)},
            'Accuracy ±1 (%)', f'Accuracy ±1 — {method}',
            os.path.join(out_dir, f'indiv_Acc1_{m_slug}_{model_type}_ws{ws}.png'),
            scenario_name, model_type, ws,
            y_lim=[0, 105])

        # B5. Improvement % — with per-method 80%/90% threshold lines
        improv_vals = sub['improvement_pct'].values
        y_h = [(0, 'Zero-Shot baseline', 'red', '--')]
        full_imp = float(improv_vals[-1]) if len(improv_vals) > 0 else 0.0
        if full_imp > 0:
            y_h.append((0.9 * full_imp,
                        f'90% ({0.9 * full_imp:.1f}%)', 'orange', '-.'))
            y_h.append((0.8 * full_imp,
                        f'80% ({0.8 * full_imp:.1f}%)', 'cyan', ':'))
        create_two_method_metric_plot(
            {method: (sizes, improv_vals)},
            'Improvement (%)', f'Improvement over Zero-Shot — {method}',
            os.path.join(out_dir, f'indiv_Improvement_{m_slug}_{model_type}_ws{ws}.png'),
            scenario_name, model_type, ws,
            y_hlines=y_h)

        # B6. Per-method accuracy combined (±1 solid, ±2 dashed)
        create_accuracy_plot(
            {method: (sizes, sub['test_acc_pm1'].values)},
            {method: (sizes, sub['test_acc_pm2'].values)},
            os.path.join(out_dir, f'indiv_Accuracy_{m_slug}_{model_type}_ws{ws}.png'),
            scenario_name, model_type, ws)

    # ── C. PER-METHOD EFFICIENCY WITH PEAK ARROW ────────────────────────────
    for method in METHODS:
        sub = df_trained[df_trained['method'] == method].sort_values('train_size')
        if len(sub) == 0:
            continue
        ...
        if valid.sum() <= 1:
            continue
        eff   = np.where(valid, improv / np.maximum(pcts, 0.01), 0)
        vsizes = sub['train_size'].values[valid]
        veff   = eff[valid]
        m_slug = method.lower().replace(' ', '_').replace('-', '_')
        _create_single_method_efficiency_plot(
            vsizes, veff, method,
            os.path.join(out_dir,
                         f'indiv_Efficiency_{m_slug}_{model_type}_ws{ws}.png'),
            scenario_name, model_type, ws)
